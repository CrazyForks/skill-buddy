...
const blockingIssues = computed(() => issues.value.filter((item) => !item.recommended))
const compliant = computed(() =>
  props.project.found && !props.project.error && blockingIssues.value.length === 0,
)

function typeIcon(type: TeamProjectRequirementStatus['type']) {
  if (type === 'bundle') return PackageCheck
  if (type === 'skill') return Sparkles
  if (type === 'instruction') return FileCog
  return ServerCog
}

function stateLabel(item: TeamProjectRequirementStatus): string {
  if (item.type === 'instruction') return t(`team.projectInstructionState.${item.state}`)
  ...
  <article class="rounded-md border px-4 py-3">
    <div class="flex items-start justify-between gap-3">
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        <Badge v-else variant="outline" class="border-amber-500/50 text-amber-700 dark:text-amber-400">
          {{ t('team.projectAttention', { n: blockingIssues.length }) }}
        </Badge>
    ...
    <div v-else-if="project.found" class="mt-3">
      <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
        ...
        <span>{{ t('team.projectRecommended', { n: project.recommended }) }}</span>
      </div>
